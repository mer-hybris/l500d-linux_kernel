...
static struct sensors_classdev sensors_light_cdev = {
	...
	.flags=2,  /* JZZ(zhizhang)@uniscope_drv configure the sensor for SENSOR_FLAG_ON_CHANGE_MODE mode 20150420 */
};

static struct sensors_classdev sensors_proximity_cdev = {
	.name = "stk3311-proximity",
	.vendor = "Sensortek",
	.version = 1,
	...
	.sensors_enable = NULL,
	.sensors_poll_delay = NULL,
	.flags=3,  /* JZZ(zhizhang)@uniscope_drv configure the sensor for SENSOR_FLAG_ON_CHANGE_MODE&SENSOR_FLAG_WAKE_UP mode 20150420 */
};
...
/*static int32_t stk3x1x_set_ps_aoffset(struct stk3x1x_data *ps_data,
		uint16_t offset);*/
...
/*
static int32_t stk3x1x_set_ps_foffset(struct stk3x1x_data *ps_data,
	uint16_t offset)
{
	uint8_t temp;
...
static int32_t stk3x1x_enable_als(struct stk3x1x_data *ps_data, uint8_t enable)
{
	...
	//kangyan 20150630 when sensor enter sleep,lsensor would be closed,so we want't to write register again
	#if 0
...
/*
#if 0
static ssize_t stk_als_lux_thd_l_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	...
	mutex_lock(&ps_data->io_lock);
	als_thd_l0_reg = i2c_smbus_read_byte_data(ps_data->client,
		STK_THDL1_ALS_REG);
	als_thd_l1_reg = i2c_smbus_read_byte_data(ps_data->client,
		STK_THDL2_ALS_REG);
	if (als_thd_l0_reg < 0) {
		dev_err(dev, "%s fail, err=0x%x", __func__, als_thd_l0_reg);
		return -EINVAL;
	}
	if (als_thd_l1_reg < 0) {
		dev_err(dev, "%s fail, err=0x%x", __func__, als_thd_l1_reg);
		return -EINVAL;
	}
...
static ssize_t stk_als_lux_thd_l_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t size)
{
	struct stk3x1x_data *ps_data =  dev_get_drvdata(dev);
	unsigned long value = 0;
...
static ssize_t stk_als_lux_thd_h_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	...
	mutex_lock(&ps_data->io_lock);
	als_thd_h0_reg = i2c_smbus_read_byte_data(ps_data->client,
		STK_THDH1_ALS_REG);
	als_thd_h1_reg = i2c_smbus_read_byte_data(ps_data->client,
		STK_THDH2_ALS_REG);
	if (als_thd_h0_reg < 0) {
		dev_err(dev, "%s fail, err=0x%x", __func__, als_thd_h0_reg);
		return -EINVAL;
	}
	if (als_thd_h1_reg < 0) {
		dev_err(dev, "%s fail, err=0x%x", __func__, als_thd_h1_reg);
		return -EINVAL;
	}
...
static ssize_t stk_als_lux_thd_h_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t size)
{
	struct stk3x1x_data *ps_data =  dev_get_drvdata(dev);
	unsigned long value = 0;
	...
	ret = kstrtoul(buf, 10, &value);
	if (ret < 0) {
		dev_err(dev, "%s:strict_strtoul failed, ret=0x%x\n",
			__func__, ret);
		return ret;
	}
	mutex_lock(&ps_data->io_lock);
