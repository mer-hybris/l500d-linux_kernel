...
const int g_TpVirtualKey[] = {TOUCH_KEY_MENU, TOUCH_KEY_BACK, TOUCH_KEY_HOME, TOUCH_KEY_SEARCH};
...
s32 DrvPlatformLyrTouchDeviceRegisterFingerTouchInterruptHandler(void)
{
    ...
    {    	
        ...
        nRetVal = request_irq(_gIrq/*MS_TS_MSG_IC_GPIO_INT*/, _DrvPlatformLyrFingerTouchInterruptHandler,
                      IRQF_TRIGGER_HIGH /* | IRQF_NO_SUSPEND *//* IRQF_TRIGGER_FALLING */,
                      "msg2xxx", NULL);
        if (nRetVal != 0)
        {
...
//    free_irq(MS_TS_MSG_IC_GPIO_INT, g_InputDevice);
    ...
    gpio_free(MSM_TS_MSG_IC_GPIO_INT);
    gpio_free(MSM_TS_MSG_IC_GPIO_RST);
