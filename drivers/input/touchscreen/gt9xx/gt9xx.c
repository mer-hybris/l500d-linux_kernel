...
//wangjing@uniscope_drv 20150707 modify for L310 TP begin
...
#endif
//wangjing@uniscope_drv 20150707 modify for L310 TP end
			   ...
			   const struct i2c_device_id *id)
{
	...
	if (gpio_is_valid(56)) {
		...
		if (err) {
			dev_err(&client->dev, "yyy reset gpio request failed");
			return err;
		}

		err = gpio_direction_output(56, 1);
		if (err) {
			dev_err(&client->dev,
				"yyy set_direction for reset gpio failed\n");
			return err;
		}
